...
Сейчас это переходная архитектура после объединения PR #6: backend уже
содержит безопасный Change Set API и строгую нормализацию, а расширенный
демонстрационный сценарий frontend (сотрудники, классификация и план задач)
временно выполняется локально в `engine.js`/`launch.js`. Следующий технический
этап — перенести эту оставшуюся логику за API и удалить дублирование правил.
...
- lint, проверка типов и 117 тестов бекенда.

Frontend-демонстрация:

- выбор области и полей проверки;
- выборочное подтверждение отдельных действий до применения;
...
- frontend пока не использует Change Set API для расширенного сценария, поэтому
  backend и локальный `engine.js` могут расходиться;

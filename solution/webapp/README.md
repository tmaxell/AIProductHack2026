...
- **Набор изменений**: `POST /change-sets` считает draft и ничего не пишет,
  `publish` применяет только подтверждённые действия и отклоняет операцию
  поверх изменившихся данных конфликтом;
- **Версии**: draft → published → superseded в SQLite, append-only история,
  diff двух версий, компенсирующий rollback через подтверждение (ADR 0001);
- **Выборочное подтверждение** каждого действия — обязательное требование
  `docs/core/PROJECT.md`;
- **14 правил нормализации**: email и телефоны компании и заявителя, ИНН, ФИО
  с инициалами, название компании, город с регистром и транслитерацией,
  бюджет, валюта, приоритет, статус, две даты. Плюс межполевая проверка
  порядка дат;
- **Сопоставление со справочником** (1400 компаний) по ИНН, email, телефону,
  названию и алиасам, с уровнем уверенности и признаками совпадения.
  Каноническое название компании берётся оттуда же;
- **Поиск дублей** по всему корпусу из 6800 заявок;
- **Работа с полной выгрузкой**: snapshot, чтение страницами, разбор партиями,
  публикация и откат относительно backend (ADR 0003);
- **AI-объяснение и разбор спорных значений** через Groq, оба по явному
  действию, с маскированием и локальной валидацией ответа (ADR 0002);
- конфигурация только из переменных окружения с проверкой на старте, lint,
  проверка типов и 165 тестов.

Ещё не сделано:

- **связи через Link и Lookup**, задачи по шаблону и назначение исполнителей
  в backend. Итоговый отчёт запуска считает их на клиенте, демонстрационно,
  round-robin и по 30 встроенным записям — его числа не сходятся с панелью;
- разбор идёт партиями по 100 записей: решения по всей выгрузке не помещаются
  в тело запроса, снятие ограничения требует массовой формы решений;

...
3. Тай-брейк / усиление: пересечение токенов `required_roles_raw`+`required_skills_raw` заявки с `roles`/`skills` из шаблонной группы (Jaccard-подобная доля пересечения) добавляется к score сходства имени.
